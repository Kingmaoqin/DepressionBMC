...
\textcolor{blue}{Using the leakage-free pipeline, we summarize performance in Table~\ref{tab:perf_summary} and Table~\ref{tab:balanced_metrics}: CatBoost and Random Forest achieved mean test ROC-AUC around 0.77 with accuracy ~0.70 across stratified folds, while GroupKFold scores were lower but still informative for assessing trial-level generalization. Balanced accuracy and class-wise metrics are derived from out-of-fold predictions, using the imbalanced class counts (347 SNRIs vs. 1121 SSRIs) to recover specificity from the reported precision/recall.}
