...
\textbf{Results:} \textcolor{red}{\sout{Among 17 binary classifiers, Random Forest achieved highest performance (accuracy, F1, precision, recall, ROC-AUC near 0.85).}} \textcolor{blue}{Across 17 classifiers and three labeling schemas, CatBoost and Random Forest achieved the highest performance (test accuracy/F1/precision/recall ~0.70-0.77, ROC-AUC ~0.78); 95\% bootstrap CIs confirmed robust generalization.} Sample-based CFs revealed both local and global feature importance of individual symptoms in medication selection.  
...
\begin{table}[!htbp]
  \centering
  \caption{Detailed drug-specific dosage distribution and classification across three labeling schemas. Note: Paroxetine was administered at 20mg exclusively across all trials, so the 50mg threshold did not affect any instances.}
  \label{tab:drug_dosage_detail}
  \begin{tabular}{|l|c|c|c|c|c|}
    \hline
    \textbf{Drug} & \textbf{Dosage (mg/day)} & \textbf{Count (N)} & \textbf{Schema A} & \textbf{Schema B} & \textbf{Schema C} \\
     & & & \textbf{(Name-based)} & \textbf{(Dosing V1)} & \textbf{(Dosing V2)} \\
    \hline
    ...
    \hline
    \multicolumn{3}{|r|}{\textbf{Total SNRI:}} & \textbf{1070} & \textbf{930} & \textbf{347} \\
    \multicolumn{3}{|r|}{\textbf{Total SSRI:}} & \textbf{398} & \textbf{538} & \textbf{1121} \\
    \hline
  \end{tabular}
\end{table}
}
...
\begin{table}[!htbp]
  \centering
  \caption{Summary of antidepressant labeling schemes and overall class counts.}
...
\caption{\textcolor{red}{\sout{Confusion matrix of random forest model trained on oversampled and one-hot encoded data classified by V2 dosage.}} \textcolor{blue}{Confusion matrix of Random Forest model on non-oversampled test data (Dosing V2 Exclusive schema), reflecting true class imbalance: 347 SNRI vs 1121 SSRI instances. SMOTE oversampling was applied only within training folds to prevent data leakage.}}
\label{fig2}
...
\begin{figure*}[!htbp]
    ...
    \begin{minipage}[b]{0.35\textwidth}
        ...
        \label{fig4b}
    \end{minipage}
    \caption{Feature importance results. (a) Local feature importance calculated from the minimum changes between the original input and 10 CFs. The most influential features contributing to the model prediction (medication selection) at the personalized level are V1-HAM-D01 (depressed mood), V1-HAM-D09 (psychomotor agitation), V1-HAM-D12 (loss of appetite), V1-HAM-D13 (tiredness/pain), and V1-HAM-D03 (suicidal thoughts or actions). (b) Global feature importance result on all instances, where the most influential feature contributing to the model prediction on medication selection at the population level is V1-HAM-D01 (depressed mood), while the least contributing one is \textcolor{red}{\sout{V1-HAM-D16 (lack of insight)}} \textcolor{blue}{V1-HAM-D16 (Weight loss)}.}
    \label{fig4}
\end{figure*}
...
The Fig. \ref{fig4}(b) quantifies the specific degree of symptom impacts, V1-HAM-D01 (Depressed mood) is identified as the most important symptom in influencing model predictions, whereas \textcolor{red}{\sout{V1-HAM-D16 (Lack of insight)}} \textcolor{blue}{V1-HAM-D16 (Weight loss)} is shown as the least important symptom.

\textcolor{blue}{To validate the clinical plausibility of our model-derived feature importance, multiple medical experts from our research team (co-authors with clinical backgrounds in mood disorders: A.G., S.L., E.T., T.S., and M.K.) independently reviewed the global and local feature importance rankings. The evaluation protocol asked experts to assess whether each highly-ranked symptom is clinically known to influence SSRI vs SNRI selection based on established pharmacological principles and clinical experience. The expert panel confirmed that symptoms such as HAM-D01 (Depressed mood), HAM-D10 (Psychic anxiety), HAM-D09 (Psychomotor agitation), and HAM-D14 (Loss of sexual interest) are indeed clinically relevant to medication class distinction, lending external validity to our model-learned associations.}
...
\textcolor{blue}{To verify the robustness of our global counterfactual feature rankings, we conducted stability experiments across 10 random seeds and all three labeling schemas (Standard, V1\_Inclusive, V2\_Exclusive). The top-5 ranked symptoms (HAM-D01 Depressed mood, HAM-D09 Psychomotor agitation, HAM-D12 Loss of appetite, HAM-D14 Loss of sexual interest, and HAM-D03 Suicidal thoughts) remained consistent across all experimental conditions, with Spearman rank correlation $\rho > 0.92$ between different random seeds and $\rho > 0.88$ across different labeling schemas. This consistency confirms that our global CF importance rankings are not artifacts of random initialization or label definition, but rather reflect stable model-learned symptom-medication associations in the pooled RCT data.}
...
\textcolor{blue}{To contextualize our counterfactual approach within the broader landscape of explainable AI methods, we conducted a comparative analysis with SHAP (SHapley Additive exPlanations) and LIME (Local Interpretable Model-agnostic Explanations). While SHAP and LIME effectively identify which features drive model predictions through attribution-based explanations, they do not inherently provide actionable guidance on how changing these features would alter outcomes—a critical capability for clinical decision support.

Fig. \ref{fig:shap_global} presents the SHAP global importance for the Standard labeling scheme with demographics, confirming that features such as HAM-D01 (Depressed mood), HAM-D09 (Psychomotor agitation), and HAM-D12 (Loss of appetite) rank highly, consistent with our CF-based global feature importance. Fig. \ref{fig:lime_local} shows aggregated LIME local importance across sampled instances, further validating that these symptoms are central to the model's decision boundary between SSRI and SNRI classes.

However, the key advantage of counterfactual explanations lies in their actionability: they quantify the minimal symptom score adjustments required to flip the predicted medication class, directly addressing the clinical question "What would need to change for this patient to be recommended a different medication class?" This what-if capability, combined with constraints that respect clinical feasibility (e.g., fixing unchangeable symptoms), is absent in SHAP and LIME, which focus on attribution rather than intervention. Together, these complementary XAI techniques reinforce the clinical plausibility and interpretability of our proposed framework, with CFs adding unique value through their intervention-oriented perspective.}
...
\textcolor{blue}{We explore the application of counterfactual reasoning in depression medication selection, focusing on both personalized and population-level analyses. Our approach utilizes CFs to investigate model-implied associations between HAM-D symptoms and RCT medication arm assignments (SSRIs/SNRIs), representing a significant advancement in combining interpretable counterfactual explanations with AI CDSSs for hypothesis generation.}
...
\textcolor{blue}{While CatBoost and Random Forest achieved comparable performance metrics (Table \ref{tab:perf_summary}), we observed that different model architectures generate divergent counterfactual explanations for the same patient instances, even when their overall prediction accuracy is similar. We expanded our DICE analysis to four additional high-performing models (CatBoost, Gradient Boosting, Extra Trees, and Stacking) from our model zoo.

This phenomenon reflects "Predictive Multiplicity": multiple models achieve similar accuracy by exploiting different correlations within the high-dimensional symptom space. For example, Random Forest may prioritize interactions between HAM-D01 (Depressed mood) and HAM-D12 (Loss of appetite), whereas CatBoost emphasizes patterns involving HAM-D09 (Psychomotor agitation) and HAM-D10 (Psychic anxiety). Since different model architectures arrive at predictions via distinct logical pathways through the feature space, the counterfactual explanations they generate—which reveal the minimal changes needed to flip predictions—naturally differ accordingly.

This finding underscores a critical implication for clinical deployment: counterfactual explanations must be model-specific. Clinicians should interpret CFs relative to the deployed model's decision logic, rather than assuming universal symptom-medication rules that generalize across all models. We retained Random Forest as our primary model not only for its strong performance but also because its DICE-generated explanations were validated by our clinical experts as most medically plausible and aligned with established pharmacological knowledge. This expert validation step is essential to ensure that model-implied symptom-medication associations align with clinical understanding before deployment in decision support systems.}
...
\section{Conclusion}
\textcolor{red}{\sout{In this paper, we present a counterfactual reasoning approach based on explainable counterfactual reasoning to investigate the causal relationship between the HAM-D scales and the categories of anti-depressant medication prescribed by clinicians. Our method employs counterfactual scenarios to simulate causal relationships, elucidating not only the causal relationship but also deriving feature importance from the disparities between CFs and the original instances.}}
\textcolor{blue}{In this paper, we present an explainable counterfactual reasoning approach to investigate model-implied associations between HAM-D symptom profiles and RCT medication arm assignments (SSRI vs SNRI). Our method employs counterfactual scenarios as model-based what-if analyses, quantifying minimal symptom adjustments that would flip predicted medication class and deriving feature importance from the differences between CFs and original instances.} In this case we found that "depressed mood" (HAM-D01) and "loss of sexual interest" (HAM-D14) were features that were most influential in determining medication selection, while "weight loss" (HAM-D16) was least important. \textcolor{red}{\sout{This approach offers valuable AI CDSSs by providing interpretations into the causal relationships and feature importance underlying clinical decision-making process.}} \textcolor{blue}{This approach offers valuable AI CDSSs by providing interpretable insights into the symptom-medication associations used by the model, supporting hypothesis generation for clinical decision-making that requires prospective validation in real-world settings.} By addressing both personalized and population level analyses, our approach enhances the ability to tailor treatments to individual patient needs while also understanding broader trends and patterns within the population.
